...
def _populate_project_object(project, data):
    ...
    if get_errors(clear=False):
        raise err.TaigaImportError(_("error importing roles"), None)
    ...
    if get_errors(clear=False):
        raise err.TaigaImportError(_("error importing memberships"),  project)
    ...
    if get_errors(clear=False):
        raise err.TaigaImportError(_("error importing lists of project attributes"), project)

    # Create default values for project attributes
    store_default_project_attributes_values(project, data)

    if get_errors(clear=False):
        raise err.TaigaImportError(_("error importing default project attributes values"), project)
    ...
    if get_errors(clear=False):
        raise err.TaigaImportError(_("error importing custom attributes"), project)
    ...
    if get_errors(clear=False):
        raise err.TaigaImportError(_("error importing sprints"), project)

    # Create user stories
    store_user_stories(project, data)

    if get_errors(clear=False):
        raise err.TaigaImportError(_("error importing user stories"), project)

    # Createer tasks
    store_tasks(project, data)

    if get_errors(clear=False):
        raise err.TaigaImportError(_("error importing tasks"), project)

    # Create issues
    store_issues(project, data)

    if get_errors(clear=False):
        raise err.TaigaImportError(_("error importing issues"), project)

    # Create wiki pages
    store_wiki_pages(project, data)

    if get_errors(clear=False):
        raise err.TaigaImportError(_("error importing wiki pages"), project)

    # Create wiki links
    store_wiki_links(project, data)

    if get_errors(clear=False):
        raise err.TaigaImportError(_("error importing wiki links"), project)

    # Create tags
    store_tags_colors(project, data)

    if get_errors(clear=False):
        raise err.TaigaImportError(_("error importing tags"), project)

    # Create timeline
    store_timeline_entries(project, data)

    if get_errors(clear=False):
        raise err.TaigaImportError(_("error importing timelines"), project)

    # Regenerate stats
    project.refresh_totals()


def store_project_form_dict(data, owner=None):
    # Validate
    if owner:
    ...
    try:
        ...
        raise err.TaigaImportError(_("unespected error importing project"), project)
